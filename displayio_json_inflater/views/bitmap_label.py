    ...
    def __init__(self, display, layout_json):
        ...
        if layout_json["view_type"] != "BitmapLabel":
            raise IncorrectTypeError(
                "view_type '{}' does not match Layout Class 'BitmapLabel'".format(
        ...
        if "attributes" in layout_json:
            ...
            if "anchored_position" in layout_json["attributes"]:
                ...
                print(self.keyword_compiler(pos[0]), self.keyword_compiler(pos[1]))
                self.label.anchored_position = (
                    self.keyword_compiler(pos[0]),
    ...
    def update_position(self):
        ...
        if "x" in layout_json["attributes"]:
            _x = self.keyword_compiler(
                layout_json["attributes"]["x"],
                {"WIDTH": _width * self._scale, "HEIGHT": _height * self._scale},
            )
            self.label.x = _x
        _y = 0
        if "y" in layout_json["attributes"]:
            _y = self.keyword_compiler(
                layout_json["attributes"]["y"],
                {"WIDTH": _width * self._scale, "HEIGHT": _height * self._scale},
            )
            self.label.y = _y
