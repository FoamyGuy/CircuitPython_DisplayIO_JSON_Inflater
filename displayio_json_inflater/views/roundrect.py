...
from displayio_json_inflater.views.view import View

REQUIRED_ATTRIBUTES = ["x", "y", "height", "width", "radius"]
    ...
    def __init__(self, display, layout_json):
        ...
        if layout_json["view_type"] != "RoundRect":
            raise IncorrectTypeError(
                "view_type '{}' does not match Layout Class 'RoundRect'".format(
        ...
        if "attributes" in layout_json:
            ...
            if "radius" in layout_json["attributes"]:
                _r = self.keyword_compiler(
                    layout_json["attributes"]["radius"],
                    {"WIDTH": _width, "HEIGHT": _height},
                )

            _x = 0
            if "x" in layout_json["attributes"]:
                _x = self.keyword_compiler(
                    layout_json["attributes"]["x"], {"WIDTH": _width, "HEIGHT": _height}
                )

            _y = 0
            if "y" in layout_json["attributes"]:
                _y = self.keyword_compiler(
                    layout_json["attributes"]["y"], {"WIDTH": _width, "HEIGHT": _height}
                )
