...
import json
import displayio
from displayio_json_inflater.layout_exceptions import (
    MissingTypeError,
    IncorrectTypeError,
    MissingSubViewsError,
)
...
class LinearLayout:
    def __init__(self, display, layout_json, portrait_orientation=True):
        ...
        if self.layout_json_obj["view_type"] != "LinearLayout":
            raise IncorrectTypeError(
                "view_type '{}' does not match Layout Class 'LinearLayout'".format(
                    self.layout_json_obj["view_type"]
                )
            )
    ...
    def _build_group_from_layout_json(self):
        ...
        for view_type in _imports_needed_dict.keys():
            if view_type == "Line":
                from displayio_json_inflater.views.line import LineView

                self._view_type_dict[view_type] = LineView
            if view_type == "RoundRect":
                from displayio_json_inflater.views.roundrect import RoundRectView

                self._view_type_dict[view_type] = RoundRectView
            if view_type == "Rect":
                from displayio_json_inflater.views.rect import RectView

                self._view_type_dict[view_type] = RectView
            if view_type == "Triangle":
                from displayio_json_inflater.views.triangle import TriangleView

                self._view_type_dict[view_type] = TriangleView
            if view_type == "SparkLine":
                from displayio_json_inflater.views.sparkline import SparkLineView

                self._view_type_dict[view_type] = SparkLineView
            if view_type == "Button":
                from displayio_json_inflater.views.button import ButtonView

                self._view_type_dict[view_type] = ButtonView
            if view_type == "Circle":
                from displayio_json_inflater.views.circle import CircleView

                self._view_type_dict[view_type] = CircleView
            if view_type == "OnDiskBitmap":
                from displayio_json_inflater.views.on_disk_bitmap import (
                    OnDiskBitmapView,
                )

                self._view_type_dict[view_type] = OnDiskBitmapView
            if view_type == "Polygon":
                from displayio_json_inflater.views.polygon import PolygonView

                self._view_type_dict[view_type] = PolygonView
            if view_type == "Image":
                from displayio_json_inflater.views.image import ImageView

                self._view_type_dict[view_type] = ImageView
            if view_type == "ProgressBar":
                from displayio_json_inflater.views.progress_bar import ProgressBarView

                self._view_type_dict[view_type] = ProgressBarView
            if view_type == "Label":
                from displayio_json_inflater.views.label import LabelView

                self._view_type_dict[view_type] = LabelView
            if view_type == "BitmapLabel":
                from displayio_json_inflater.views.bitmap_label import LabelView
        ...
        for index, view in enumerate(self.layout_json_obj["sub_views"]):
            ...
            if view["view_type"] != "Label":
                view_layout = self._view_type_dict[view["view_type"]](
                    self._display, view
                )
                ...
                if self.portrait_orientation:
                    ...
                    try:
                        self._prev_view_end = (
                            view_layout.view.y + view_layout.view.height
                        )
                    except AttributeError as e:
                        print(e)
                        try:
                            self._prev_view_end = (
                                view_layout.view.y + view_layout.view._height
                            )
                        except AttributeError as e:
                            print(e)
                            self._prev_view_end = (
                                view_layout.view.y + view_layout.height
                            )
                else:
            ...
            if view["view_type"] == "Label":
                label_view = self._view_type_dict[view["view_type"]](
                    self._display, view
                )
                ...
                if self.portrait_orientation:
                    ...
                    print(
                        "setting y to: {}".format(
                            self._prev_view_end + label_view.view.y
                        )
                    )
                    label_view.view.anchored_position = (
                        label_view.view.x,
                        self._prev_view_end + label_view.view.y,
                    )
